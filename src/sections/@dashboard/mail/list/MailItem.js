...
const StyledName = styled((props) => <Typography component="span" noWrap {...props} />)(({ theme }) => ({
...
const StyledTextContent = styled((props) => <Typography component="span" noWrap {...props} />)(({ theme }) => ({
  ...theme.typography.body2,
  marginRight: theme.spacing(2),
  flexGrow: 1,
}));
...
export default function MailItem({ mail, dense, selected, onSelect, onDeselect, getLabel, ...other }) {
    ...
    >
      {isDesktop && (
        <Stack direction="row" alignItems="center" sx={{ mr: 2 }}>
          <Checkbox checked={selected} onChange={(event) => handleChangeCheckbox(event.target.checked)} />
