...
export default function Mail() {
  ...
  const handleDeselectMail = (mailId) => {
  ...
  return (
    <>
      <Container maxWidth={themeStretch ? false : 'xl'}>
        ...
        >
          <MailNav items={labels} openNav={openNav} onCloseNav={handleCloseNav} onOpenCompose={handleOpenCompose} />

          <Stack flexGrow={1}>
            {mail ? (
              <>
                <MailHeaderDetails mailFrom={mail.from} mailTo={mail.to} createdAt={mail.createdAt} />
                <MailDetails subject={mail.subject} message={mail.message} attachments={mail.attachments} />
              </>
            ) : (
              <>
                <MailHeader
